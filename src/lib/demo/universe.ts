...
const REGION_SEEDS: RegionSeed[] = [
  ...
  // =========================
  {
    ...
    scenario: "wildfire",
    centre: { lat: 39.2, lon: -121.1 },
    backhaulPool: ["sat", "4g"],
    profileBias: { aaq: 0.55, tsr: 0.2, sss: 0.2, sms: 0.03, svs: 0.02 },
    siteHints: {
      structure_id: "asset-wildfire-ca-01",
      asset_type: "slope",
      note: "wildfire perimeter / smoke corridors",
    },
    income: "high",
    area_km2: 165000,
    spacing_km: 5.0,
  ...
  },
  {
    ...
    scenario: "wildfire",
    centre: { lat: -36.5, lon: 146.0 },
    backhaulPool: ["sat", "4g"],
    profileBias: { aaq: 0.6, tsr: 0.18, sss: 0.18, sms: 0.02, svs: 0.02 },
    siteHints: {
      structure_id: "asset-wildfire-au-01",
      asset_type: "slope",
      note: "bushfire detection + smoke transport",
    },
    income: "high",
    area_km2: 120000,
    spacing_km: 6.0,
  ...
  // =========================
  {
    ...
    profileBias: { sms: 0.55, tsr: 0.2, svs: 0.2, aaq: 0.03, sss: 0.02 },
  ...
  },
  {
    ...
    scenario: "earthquake",
    centre: { lat: 40.75, lon: 31.6 },
    backhaulPool: ["4g", "sat"],
    profileBias: { sms: 0.6, tsr: 0.2, svs: 0.15, sss: 0.03, aaq: 0.02 },
  ...
  },
  {
    ...
    scenario: "earthquake",
    centre: { lat: 27.7, lon: 85.32 },
    backhaulPool: ["sat", "4g"],
    profileBias: { sms: 0.55, tsr: 0.25, sss: 0.1, svs: 0.08, aaq: 0.02 },
  ...
  },
  {
    ...
    backhaulPool: ["4g", "sat"],
    profileBias: { sms: 0.5, svs: 0.2, tsr: 0.25, aaq: 0.03, sss: 0.02 },
  ...
  // =========================
  {
    ...
    scenario: "landslide",
    centre: { lat: 30.3, lon: 78.05 },
    backhaulPool: ["4g", "sat"],
    profileBias: { sss: 0.45, sms: 0.35, tsr: 0.1, svs: 0.07, aaq: 0.03 },
  ...
  },
  {
    ...
    scenario: "landslide",
    centre: { lat: 16.41, lon: 120.6 },
    backhaulPool: ["4g", "sat"],
    profileBias: { sss: 0.5, sms: 0.25, aaq: 0.05, svs: 0.05, tsr: 0.15 },
  ...
  },
  {
    ...
    scenario: "landslide",
    centre: { lat: 60.65, lon: 7.3 },
    backhaulPool: ["sat", "4g"],
    profileBias: { sms: 0.35, sss: 0.35, tsr: 0.2, svs: 0.08, aaq: 0.02 },
  ...
  // =========================
  {
    ...
    backhaulPool: ["sat", "4g"],
    profileBias: { sms: 0.4, aaq: 0.3, tsr: 0.15, svs: 0.1, sss: 0.05 },
  ...
  },
  {
    ...
    scenario: "volcano",
    centre: { lat: -4.2, lon: 152.2 },
    backhaulPool: ["sat"],
    profileBias: { sms: 0.45, aaq: 0.25, tsr: 0.2, svs: 0.05, sss: 0.05 },
  ...
  // =========================
  {
    ...
    profileBias: { sss: 0.7, aaq: 0.1, tsr: 0.15, svs: 0.03, sms: 0.02 },
  ...
  },
  {
    ...
    scenario: "agriculture",
    centre: { lat: -0.3, lon: 36.1 },
    backhaulPool: ["4g"],
    profileBias: { sss: 0.8, tsr: 0.15, aaq: 0.05 },
  ...
  // =========================
  {
    ...
    profileBias: { svs: 0.65, tsr: 0.2, aaq: 0.1, sss: 0.03, sms: 0.02 },
  ...
  },
  {
    ...
    backhaulPool: ["4g"],
    profileBias: { svs: 0.55, tsr: 0.3, aaq: 0.1, sms: 0.03, sss: 0.02 },
  ...
  // =========================
  {
    ...
    profileBias: { aaq: 0.7, tsr: 0.15, svs: 0.1, sss: 0.03, sms: 0.02 },
...
// Drop-in: add spatial layouts (cluster/grid/corridor/ring)
...
function estimateCoverageRadiusKm(n: number) {
  return clamp(0.8 + Math.sqrt(n) * 0.35, 1.0, 45.0);
}
...
function placeCorridor(rnd: () => number, centre: { lat: number; lon: number }, n: number) {
  const radiusKm = estimateCoverageRadiusKm(n);
  const lengthKm = clamp(radiusKm * 2.2, 4, 120);
  const widthKm = clamp(radiusKm * 0.35, 0.4, 12);
...
function placeNode(rnd: () => number, centre: { lat: number; lon: number }, layout: Layout, nForGateway: number) {
  switch (layout) {
    case "grid":
...
export function buildDemoUniverse(scale: DemoScale): DemoUniverse {
  ...
  for (let g = 0; g < gateways.length; g++) {
    ...
    for (let i = 0; i < perGw[g]; i++) {
      ...
      // ✅ FIX: do NOT spread `region.siteHints`.
      // Spreading a possibly-optional object makes TS infer optional keys under strict builds,
      // causing Amplify `next build` to fail.
      const structure: NodeRegistryItem["structure"] =
        profile === "svs" || profile === "sms" || profile === "sss"
          ? region.siteHints
          : undefined;
